...
#elif (IS_ENABLED(CONFIG_SOC_EXYNOS9830) || IS_ENABLED(CONFIG_SOC_EXYNOS2100) || IS_ENABLED(CONFIG_SOC_S5E9925))
...
static int dma_new(struct snd_soc_component *component, struct snd_soc_pcm_runtime *rtd)
{
	...
	dp_debug_sfr = ioremap(0x10A80000, 0x7000);
#endif
	return ret;
}
...
#elif defined(CONFIG_SOC_EXYNOS2100) || defined(CONFIG_SOC_S5E9925)
	dp_ado_extcon = devm_extcon_dev_allocate(dev, extcon_id);
	if (IS_ERR(dp_ado_extcon)) {
		dev_err(dev, "Failed to allocate dp audio extcon\n");
		return -EINVAL;
