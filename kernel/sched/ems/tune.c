			...
			struct emstune_prefer_cpu *prefer_cpu)
{
	...
	if (!of_property_read_string(dn, "small-task", &buf))
...
static ssize_t
show_sched_boost(struct kobject *k, struct kobj_attribute *attr, char *buf)
{
	return sprintf(buf, "%d\n", sched_boost);
}

static ssize_t
store_sched_boost(struct kobject *k, struct kobj_attribute *attr,
...
__ATTR(sched_boost, 0644, show_sched_boost, store_sched_boost);
...
static ssize_t
show_task_boost(struct kobject *k, struct kobj_attribute *attr, char *buf)
{
	return sprintf(buf, "%d\n", task_boost);
}

static ssize_t
store_task_boost(struct kobject *k, struct kobj_attribute *attr,
				const char *buf, size_t count)
{
	int boost;
 ...
 ******************************************************************************/
static ssize_t
show_status(struct kobject *k, struct kobj_attribute *attr, char *buf)
{
...
static ssize_t
show_req_level(struct kobject *k, struct kobj_attribute *attr, char *buf)
{
	int ret;
	unsigned long flags;
...
static ssize_t
store_req_level(struct kobject *k, struct kobj_attribute *attr,
				const char *buf, size_t count)
{
	int level;
...
static struct kobj_attribute req_level_attr =
__ATTR(req_level, 0644, show_req_level, store_req_level);

static ssize_t
show_reset(struct kobject *k, struct kobj_attribute *attr, char *buf)
{
	return sprintf(buf, "usage\n# <mode> <level> > reset\n");
}

static ssize_t
store_reset(struct kobject *k, struct kobj_attribute *attr,
				const char *buf, size_t count)
{
	int mode, level;
...
show_cur_set(struct kobject *k, struct kobj_attribute *attr, char *buf)
{
	...
	if (emstune.boot.ongoing)
		ret += sprintf(buf + ret, "Booting exclusive mode\n");
	else
		ret += sprintf(buf + ret, "%s mode : level%d [mode=%d level=%d]\n",
	...
	for (group = 0; group < CGROUP_COUNT; group++)
		ret += sprintf(buf + ret, "%4s", task_cgroup_simple_name[group]);
	ret += sprintf(buf + ret, "\n");
	for (group = 0; group < CGROUP_COUNT; group++)
	...
	ret += sprintf(buf + ret, "[active-weight]\n");
	ret += sprintf(buf + ret, "     ");
	for (group = 0; group < CGROUP_COUNT; group++)
		ret += sprintf(buf + ret, "%4s", task_cgroup_simple_name[group]);
	ret += sprintf(buf + ret, "\n");
	...
	ret += sprintf(buf + ret, "[cpufreq gov]\n");
	ret += sprintf(buf + ret, "pelt boost\n");
	ret += sprintf(buf + ret, "      dbs  boost\n");
	for_each_possible_cpu(cpu) {
		ret += sprintf(buf + ret, "cpu%d:%2d %4d\n", cpu,
				cur_set->cpufreq_gov.dis_buck_share[cpu],
				cur_set->cpufreq_gov.pelt_boost[cpu]);
	}
	ret += sprintf(buf + ret, "\n");
	ret += sprintf(buf + ret, "pelt margin\n");
	ret += sprintf(buf + ret, "   margin    freq\n");
	for_each_possible_cpu(cpu) {
		if (cur_set->cpufreq_gov.split_pelt_margin_freq[cpu] == INT_MAX)
			ret += sprintf(buf + ret, "cpu%d:%4d %7s\n",
				cpu, cur_set->cpufreq_gov.split_pelt_margin[cpu],
				"INF");
		else
			ret += sprintf(buf + ret, "cpu%d:%4d %7u\n",
				cpu, cur_set->cpufreq_gov.split_pelt_margin[cpu],
				cur_set->cpufreq_gov.split_pelt_margin_freq[cpu]);
	}
	ret += sprintf(buf + ret, "\n");

	ret += sprintf(buf + ret, "up rate limit\n");
	ret += sprintf(buf + ret, "     rate    freq\n");
	for_each_possible_cpu(cpu) {
	...
	ret += sprintf(buf + ret, "\n");
	for (group = 0; group < CGROUP_COUNT; group++)
		ret += sprintf(buf + ret, "%4d", cur_set->ontime.enabled[group]);
	ret += sprintf(buf + ret, "\n\n");
	{
		struct ontime_dom *dom;
		...
		if (!list_empty(cur_set->ontime.p_dom_list)) {
			list_for_each_entry(dom, cur_set->ontime.p_dom_list, node) {
				...
				ret += sprintf(buf + ret, " upper boundary : %4lu (%3d%%)\n",
					dom->upper_boundary, dom->upper_ratio);
				ret += sprintf(buf + ret, " lower boundary : %4lu (%3d%%)\n",
	...
	ret += sprintf(buf + ret, "[cpus binding]\n");
	...
	ret += sprintf(buf + ret, "[tex]\n");
	ret += sprintf(buf + ret, "mask : %#x\n",
			*(unsigned int *)cpumask_bits(&cur_set->tex.mask));
	ret += sprintf(buf + ret, "qjump: %d\n", cur_set->tex.qjump);
	ret += sprintf(buf + ret, "prio : %d\n", cur_set->tex.prio);
	ret += sprintf(buf + ret, "suppress : %d\n", cur_set->tex.suppress);
	ret += sprintf(buf + ret, "\n");
	for (group = 0; group < CGROUP_COUNT; group++)
		ret += sprintf(buf + ret, "%4s", task_cgroup_simple_name[group]);
	ret += sprintf(buf + ret, "\n");
	for (group = 0; group < CGROUP_COUNT; group++)
		ret += sprintf(buf + ret, "%4d", cur_set->tex.enabled[group]);
	ret += sprintf(buf + ret, "\n\n");
	...
	ret += sprintf(buf + ret, "[ecs]\n");
	{
		...
		if (!list_empty(&cur_set->ecs.domain_list)) {
			list_for_each_entry(domain, &cur_set->ecs.domain_list, node) {
				ret += sprintf(buf + ret, " domain id       : %d\n",
					domain->id);
				ret += sprintf(buf + ret, " threshold ratio : %u\n",
	...
	ret += sprintf(buf + ret, "[prefer cpu]\n");
	for (group = 0; group < CGROUP_COUNT; group++)
		ret += sprintf(buf + ret, "%4s", task_cgroup_simple_name[group]);
	ret += sprintf(buf + ret, "\n");
	for (group = 0; group < CGROUP_COUNT; group++)
	...
	ret += sprintf(buf + ret, "\n");
	ret += sprintf(buf + ret, "small-task : %x",
		*(unsigned int *)cpumask_bits(&cur_set->prefer_cpu.small_task_mask));
	ret += sprintf(buf + ret, "\n\n");

	ret += sprintf(buf + ret, "[new task util ratio]\n");
	for (group = 0; group < CGROUP_COUNT; group++)
		ret += sprintf(buf + ret, "%4s", task_cgroup_simple_name[group]);
	ret += sprintf(buf + ret, "\n");
	for (group = 0; group < CGROUP_COUNT; group++)
		ret += sprintf(buf + ret, "%4d", cur_set->ntu.ratio[group]);
	ret += sprintf(buf + ret, "\n\n");

	ret += sprintf(buf + ret, "[htask_boost]\n");
	ret += sprintf(buf + ret, "boost_step: %d\n", cur_set->stt.boost_step);
	ret += sprintf(buf + ret, "htask_cnt: %d\n", cur_set->stt.htask_cnt);
	for (group = 0; group < CGROUP_COUNT; group++)
		ret += sprintf(buf + ret, "%4s", task_cgroup_simple_name[group]);
	ret += sprintf(buf + ret, "\n");
	for (group = 0; group < CGROUP_COUNT; group++)
		ret += sprintf(buf + ret, "%4d", cur_set->stt.htask_enable[group]);
	ret += sprintf(buf + ret, "\n\n");

	ret += sprintf(buf + ret, "[fclamp]\n");
	for (group = 0; group < CGROUP_COUNT; group++)
		ret += sprintf(buf + ret, "%4s", task_cgroup_simple_name[group]);
	ret += sprintf(buf + ret, "\n");
	for (group = 0; group < CGROUP_COUNT; group++)
		ret += sprintf(buf + ret, "%4d", cur_set->fclamp.monitor_group[group]);
	ret += sprintf(buf + ret, "\n\n");
	ret += sprintf(buf + ret, "     |-------- min -------||-------- max -------|\n");
	ret += sprintf(buf + ret, "        freq  period ratio    freq  period ratio\n");
	for_each_possible_cpu(cpu) {
		ret += sprintf(buf + ret, "cpu%d: %7d %6d %5d  %7d %6d %5d\n",
...
static int emstune_sysfs_init(struct kobject *ems_kobj)
{
	struct kobject *emstune_kobj = kobject_create_and_add("emstune", ems_kobj);

	if (!emstune_kobj)
		return -EINVAL;

	if (sysfs_create_file(emstune_kobj, &status_attr.attr))
		return -ENOMEM;

	if (sysfs_create_file(emstune_kobj, &req_mode_attr.attr))
		return -ENOMEM;

	if (sysfs_create_file(emstune_kobj, &req_level_attr.attr))
		return -ENOMEM;

	if (sysfs_create_file(emstune_kobj, &reset_attr.attr))
		return -ENOMEM;
	...
	if (sysfs_create_file(emstune_kobj, &sched_boost_attr.attr))
		return -ENOMEM;

	if (sysfs_create_file(emstune_kobj, &task_boost_attr.attr))
		return -ENOMEM;
...
int emstune_init(struct kobject *ems_kobj, struct device_node *dn)
{
	...
	if (emstune_sysfs_init(ems_kobj))
