				     ...
				     struct device_attribute *attr, const char *buf, size_t size)
{
	...
	ret = kstrtou8(buf, 2, &enable);
	if (ret) {
		pr_err("[GRIP_U] invalid argument\n");
		return size;
	}

	pr_info("[GRIP_U] new_value=%d\n", (int)enable);
...
static DEVICE_ATTR(enable2, S_IRUGO | S_IWUSR | S_IWGRP,
		   isg6320_noti_enable_show, isg6320_noti_enable_store);

static struct attribute *isg6320_noti_attributes[] = {
	&dev_attr_enable2.attr,
	NULL,
};

static struct attribute_group isg6320_noti_attribute_group = {
	.attrs = isg6320_noti_attributes,
};
...
static void isg6320_check_first_working(struct isg6320_data *data)
{
...
static void cal_work_func(struct work_struct *work)
{
	...
	if (data->abnormal_mode && data->enable == ON) {
		...
		if (data->is_unknown_mode == UNKNOWN_ON && data->motion)
			isg6320_check_first_working(data);
		if (ret < 0) {
			schedule_delayed_work(&data->cal_work, msecs_to_jiffies(SHCEDULE_INTERVAL));
			return;
		}
					     ...
					     unsigned long action, void *data)
{
	...
	pr_info("[GRIP_%d] src %d id %d attach %d\n", pdata->ic_num,
			  ...
			  const struct i2c_device_id *id)
{
	...
	if (data->ic_num == 0) {
	    ...
	    if (ret < 0) {
	    	pr_err("[GRIP_U] failed to create symlink for noti (%d)\n", ret);
	    	goto err_create_symlink_noti;
	    }
        
	    ret = sysfs_create_group(&data->noti_input_dev->dev.kobj, &isg6320_noti_attribute_group);
	    if (ret < 0) {
	    	pr_err("[GRIP_U] failed to create sysfs group for noti(%d)\n", ret);
...
err_sensor_register:
	sysfs_remove_group(&noti_input_dev->dev.kobj, &isg6320_noti_attribute_group);
