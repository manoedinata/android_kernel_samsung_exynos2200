...
int dpu_init_debug(struct exynos_drm_crtc *exynos_crtc)
{
	...
	debug_event = debugfs_create_file("event", 0444, crtc->debugfs_entry,
			exynos_crtc, &dpu_event_fops);
	if (!debug_event) {
		DRM_ERROR("failed to create debugfs event file\n");
		goto err_event_log;
	}

	if (!debugfs_create_file("recovery", 0644, crtc->debugfs_entry, exynos_crtc,
				&recovery_fops)) {
		DRM_ERROR("failed to create debugfs recovery file\n");
		goto err_debugfs;
	}

	debugfs_create_u32("underrun_cnt", 0664,
			crtc->debugfs_entry, &exynos_crtc->d.underrun_cnt);
